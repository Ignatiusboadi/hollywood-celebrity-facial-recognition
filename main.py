...
import timm
import torchmetrics
from Dataset import ImageDataset, class_mapping
...
def main():
    ...
    def get_metrics(model, ims, gts, loss_fn, epoch_loss, epoch_acc, epoch_f1, f1_score):
        ...
        # Correct the ambiguous tensor comparison
        epoch_acc += (torch.argmax(preds, dim=1) == gts).sum().item()  # Sum correct predictions
